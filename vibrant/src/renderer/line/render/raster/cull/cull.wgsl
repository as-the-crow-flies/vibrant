...
@group(0) @binding(3) var<storage, read_write> LINE_CULL: array<u32>;

@group(1) @binding(0) var OPACITY: texture_2d<f32>;
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let level = log2(max(delta.x, delta.y)) - 1.0;

    let sample = 0.5 + 0.125 * (p00 + p10 + p01 + p11);

    let culled = textureSampleLevel(OPACITY, SAMPLER, vec2<f32>(sample.x, 1.0 - sample.y), level).x == 1.0;

    LINE_CULL[instance_index] = u32(culled);
}
