...
pub struct TractogramOcclusionShading {
    pipeline: RenderPipeline,
}

impl TractogramOcclusionShading {
    pub fn new(gpu: &Gpu) -> Self {
        let label = Some(type_name::<Self>());

        let shading_module = gpu.shader(&(Environment::wgsl() + include_str!("occlusion.wgsl")));
    ...
    pub fn render(
        ...
        pass.set_bind_group(0, frame.occlusion().volume().binding(), &[]);
