...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let radius_2 = pow(length(transform * vec4<f32>(ENVIRONMENT.settings.streamline_radius, 0.0, 0.0, 0.0)), 2.0);
    ...
    // Quilez Capsule SDF
    let dba_inv = 1.0 / dot(delta, delta);

    while (distance_left > 0.0) {
        let increment = min(minimum(distance_to_next_voxel_boundary), distance_left);
        let voxel = floor(end.xyz - distance_left * direction);
        let voxel_offset = voxel - start + 1.0 / 6.0;

        var bitmask = 0u;
        var bit = 1u;

        for (var z=0.0; z<1.0; z+=third) {
            for (var y=0.0; y<1.0; y+=third) {
                for (var x=0.0; x<1.0; x+=third) {
                    let pa = voxel_offset + vec3<f32>(x, y, z);
                    let h = saturate(dot(pa, delta) * dba_inv);
                    let sd_2 = length_2(pa - delta * h);
                    bitmask |= bit * u32(sd_2 < radius_2);
                    bit <<= 1u;
                }
            }
        }

        atomicOr(&VOLUME[u32(voxel.z)][u32(voxel.y)][u32(voxel.x)], bitmask);
